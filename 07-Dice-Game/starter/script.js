...
//       // Show 'currentScore' of 'activePlayer'
//       document.querySelector(`#current--${activePlayer}`).textContent =
//         currentScore;
//     } else {
//       // Switch the player
...
const switchPlayer = function () {
  if (playing) {
    // Put CURRENT SCORE = 0 & DISPLAY IT
    currentScore = 0;
    document.querySelector(`#current--${activePlayer}`).textContent =
      currentScore;
    ...
    // APPLY 'ACTIVE' Class to the ACTIVE PLAYER
    player0El.classList.toggle('player--active');
    player1El.classList.toggle('player--active');
  }
};
...
btnRoll.addEventListener('click', function () {
  if (playing) {
    ...
    // 2] Generate RANDOM number and SHOW IT
    let randomNumber = Math.trunc(Math.random() * 6) + 1;
    dice.src = `dice-${randomNumber}.png`;

    // 3] Add to the CURRENT SCORE
    currentScore += randomNumber;

    // 4] Check if CURRENT SCORE == 1 or NOT
    if (randomNumber !== 1) {
      // if CURRENT SCORE = 0
      document.querySelector(`#current--${activePlayer}`).textContent =
        currentScore;
